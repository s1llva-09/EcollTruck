...
public class MainActivity extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (itemId == android.R.id.home) {
            ...
            startActivity(intent);
            finish(); // Finaliza a MainActivity para que o usuário não possa voltar a ela pressionando o botão de voltar do sistema
            return true;
        } else if (itemId == R.id.action_call) {
            // Ação do botão de telefone
            Intent intent = new Intent(Intent.ACTION_DIAL);
            intent.setData(Uri.parse("tel:75981585726"));
            startActivity(intent);
            return true;
        } else if (itemId == R.id.action_sobre_nos) {
            // Ação do botão de "Sobre nós"
