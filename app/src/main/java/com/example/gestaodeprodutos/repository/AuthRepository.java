...
import com.example.gestaodeprodutos.view.EmpresaRegistro;
...
public class AuthRepository {
    ...
    // ATENÇÃO: Use a chave pública (anon key) do Supabase.
    private final String API_KEY = "sb_publishable_uOs4LwI-ku2zHwmQN7qksw_SSXnVLIr";
    ...
    public void registrarUsuario(String name, String email, String password, String cpf, String phone, String birthDate, 
        ...
        api.registrar(API_KEY, CONTENT, body).enqueue(new Callback<AuthResponse>() {
            @Override
            public void onResponse(Call<AuthResponse> call, Response<AuthResponse> response) {
                if (response.isSuccessful()) {
                    result.setValue(response.body());
                    ...
                    try {
                        ...
                        if (errorBody.contains("duplicate key value") || errorBody.contains("User already registered")) {
                            ...
                            error.setValue("Erro ao registrar: " + errorBody);
                        }
                    } catch (IOException e) {
    ...
    public void registrarEmpresa(String cnpj, String nomeResponsavel, String email, String phone, String cidade, String estado, String senha,
        ...
        EmpresaRegistro body = new EmpresaRegistro(cnpj, nomeResponsavel, email, phone, cidade, estado, senha);

        api.registrarEmpresa(API_KEY, CONTENT, body).enqueue(new Callback<AuthResponse>() {
            @Override
            public void onResponse(Call<AuthResponse> call, Response<AuthResponse> response) {
                if (response.isSuccessful()) {
                    result.setValue(response.body());
                    ...
                    try {
                        ...
                        if (errorBody.contains("duplicate key value") || errorBody.contains("User already registered")) {
                            ...
                            error.setValue("Erro ao registrar empresa: " + errorBody);
                        }
                    } catch (IOException e) {
                        Log.e(TAG, "Error parsing registration error body", e);
                        error.setValue("Ocorreu um erro inesperado.");
